# Tabulated atomic masses and coherent neutron scattering lengths.
#
# Data provenance
# ---------------
# These tables were imported from the user-supplied Phunny.jl source tree.
# Phunny labels its natural-abundance mass table as PubChem-derived and its
# coherent neutron scattering-length table as NIST-derived.  The isotope mass
# table is intentionally limited to the isotopes present in that source.
#
# Units:
#   masses                         : atomic mass units (amu, u)
#   coherent scattering lengths   : femtometres (fm)
#
# The tables are implementation details.  Public access is through
# `lookup_species(:O)` and `lookup_species(:O, 18)`.

const _MASS_U = Dict{Symbol,Float64}(
...
const _MASS_ISO_U = Dict{Tuple{Symbol,Int},Float64}(
...
const _BCOH_FM = Dict{Symbol,Float64}(
...
const _BCOH_ISO_FM = Dict{Tuple{Symbol,Int},Float64}(
